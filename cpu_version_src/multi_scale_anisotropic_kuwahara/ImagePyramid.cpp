...
ImagePyramid::ImagePyramid()
    : nb_level_(3), interpolation_flag_(cv::INTER_CUBIC) {
  this->pyramid_levels_ = std::vector<cv::Mat>();
};

ImagePyramid::ImagePyramid(const int nb_level,
...
void ImagePyramid::build_pyramid(cv::Mat& image) {
  ...
  for (int i = 0; i < this->nb_level_; i++) {
    ...
    cv::resize(image, image, cv::Size(cols, rows),
               this->scale_factor_horizontal, this->scale_factor_vertical,
               this->interpolation_flag_);
    this->pyramid_levels_.push_back(image);
  }
};

void ImagePyramid::clear_pyramid() { this->pyramid_levels_.clear(); };
